...
static duk_ret_t dukky_range_setStart(duk_context *ctx)
{
	...
	if (dukky_argc > 1) {
		if (!duk_is_number(ctx, 1)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 1, "offset");
		}
	}
...
static duk_ret_t dukky_range_collapse(duk_context *ctx)
{
	...
	if (dukky_argc > 0) {
		if (!duk_is_boolean(ctx, 0)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_bool_type, 0, "toStart");
		}
	}
	/* Get private data for method */
	range_private_t *priv = NULL;
...
static duk_ret_t dukky_range_compareBoundaryPoints(duk_context *ctx)
{
	...
	if (dukky_argc > 0) {
		if (!duk_is_number(ctx, 0)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 0, "how");
		}
	}
	if (dukky_argc > 1) {
		/* unhandled type check */
