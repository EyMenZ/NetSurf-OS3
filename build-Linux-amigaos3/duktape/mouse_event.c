...
#line 77 "javascript/duktape/netsurf.bnd"
}
...
static duk_ret_t dukky_mouse_event_initMouseEvent(duk_context *ctx)
{
	...
	if (dukky_argc < 15) {
		/* not enough arguments */
		duk_error(ctx, DUK_RET_TYPE_ERROR, dukky_error_fmt_argument, 15, dukky_argc);
	...
	if (dukky_argc > 9) {
		if (!duk_is_boolean(ctx, 9)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_bool_type, 9, "ctrlKeyArg");
		}
	}
	if (dukky_argc > 10) {
		if (!duk_is_boolean(ctx, 10)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_bool_type, 10, "altKeyArg");
		}
	}
	if (dukky_argc > 11) {
		if (!duk_is_boolean(ctx, 11)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_bool_type, 11, "shiftKeyArg");
		}
	}
	if (dukky_argc > 12) {
		if (!duk_is_boolean(ctx, 12)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_bool_type, 12, "metaKeyArg");
		}
	}
	if (dukky_argc > 13) {
		if (!duk_is_number(ctx, 13)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 13, "buttonArg");
		}
	}
	if (dukky_argc > 14) {
		/* unhandled type check */
