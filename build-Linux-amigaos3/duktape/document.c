...
static duk_ret_t dukky_document_importNode(duk_context *ctx)
{
	...
	if (dukky_argc > 1) {
		if (!duk_is_boolean(ctx, 1)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_bool_type, 1, "deep");
		}
	}
	/* Get private data for method */
	document_private_t *priv = NULL;
...
static duk_ret_t dukky_document_createNodeIterator(duk_context *ctx)
{
	...
	if (dukky_argc > 1) {
		if (!duk_is_number(ctx, 1)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 1, "whatToShow");
		}
	}
	if (dukky_argc > 2) {
		/* unhandled type check */
...
static duk_ret_t dukky_document_execCommand(duk_context *ctx)
{
	...
	if (dukky_argc > 1) {
		if (!duk_is_boolean(ctx, 1)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_bool_type, 1, "showUI");
		}
	}
	if (dukky_argc > 2) {
