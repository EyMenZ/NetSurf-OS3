...
#line 82 "javascript/duktape/netsurf.bnd"
}
...
static duk_ret_t dukky_wheel_event_initWheelEvent(duk_context *ctx)
{
	...
	if (dukky_argc < 16) {
		...
		duk_error(ctx, DUK_RET_TYPE_ERROR, dukky_error_fmt_argument, 16, dukky_argc);
	...
	if (dukky_argc > 5) {
		if (!duk_is_number(ctx, 5)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 5, "screenXArg");
		}
	}
	if (dukky_argc > 6) {
		if (!duk_is_number(ctx, 6)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 6, "screenYArg");
		}
	}
	if (dukky_argc > 7) {
		if (!duk_is_number(ctx, 7)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 7, "clientXArg");
		}
	}
	if (dukky_argc > 8) {
		if (!duk_is_number(ctx, 8)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 8, "clientYArg");
		}
	}
	if (dukky_argc > 9) {
		if (!duk_is_number(ctx, 9)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 9, "buttonArg");
		}
	}
	if (dukky_argc > 10) {
		/* unhandled type check */
	...
	if (dukky_argc > 12) {
		if (!duk_is_number(ctx, 12)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 12, "deltaXArg");
		}
	}
	if (dukky_argc > 13) {
		if (!duk_is_number(ctx, 13)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 13, "deltaYArg");
		}
	}
	if (dukky_argc > 14) {
		if (!duk_is_number(ctx, 14)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 14, "deltaZArg");
		}
	}
	if (dukky_argc > 15) {
		if (!duk_is_number(ctx, 15)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 15, "deltaMode");
		}
	}
