...
#line 86 "javascript/duktape/netsurf.bnd"
}
...
static duk_ret_t dukky_message_event_initMessageEvent(duk_context *ctx)
{
	...
	if (dukky_argc > 1) {
		if (!duk_is_boolean(ctx, 1)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_bool_type, 1, "bubbles");
		}
	}
	if (dukky_argc > 2) {
		if (!duk_is_boolean(ctx, 2)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_bool_type, 2, "cancelable");
		}
	}
	if (dukky_argc > 4) {
