...
static duk_ret_t dukky_canvas_rendering_context2d_scale(duk_context *ctx)
{
	...
	if (dukky_argc > 0) {
		if (!duk_is_number(ctx, 0)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 0, "x");
		}
	}
	if (dukky_argc > 1) {
		if (!duk_is_number(ctx, 1)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 1, "y");
		}
	}
	/* Get private data for method */
	canvas_rendering_context2d_private_t *priv = NULL;
...
static duk_ret_t dukky_canvas_rendering_context2d_rotate(duk_context *ctx)
{
	...
	if (dukky_argc > 0) {
		if (!duk_is_number(ctx, 0)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 0, "angle");
		}
	}
	/* Get private data for method */
	canvas_rendering_context2d_private_t *priv = NULL;
...
static duk_ret_t dukky_canvas_rendering_context2d_transform(duk_context *ctx)
{
	...
	if (dukky_argc < 6) {
		/* not enough arguments */
		duk_error(ctx, DUK_RET_TYPE_ERROR, dukky_error_fmt_argument, 6, dukky_argc);
	...
	if (dukky_argc > 0) {
		if (!duk_is_number(ctx, 0)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 0, "a");
		}
	}
	if (dukky_argc > 1) {
		if (!duk_is_number(ctx, 1)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 1, "b");
		}
	}
	if (dukky_argc > 2) {
		if (!duk_is_number(ctx, 2)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 2, "c");
		}
	}
	if (dukky_argc > 3) {
		if (!duk_is_number(ctx, 3)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 3, "d");
		}
	}
	if (dukky_argc > 4) {
		if (!duk_is_number(ctx, 4)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 4, "e");
		}
	}
	if (dukky_argc > 5) {
		if (!duk_is_number(ctx, 5)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 5, "f");
		}
	}
	/* Get private data for method */
	canvas_rendering_context2d_private_t *priv = NULL;
...
static duk_ret_t dukky_canvas_rendering_context2d_createLinearGradient(duk_context *ctx)
{
	...
	if (dukky_argc < 4) {
		/* not enough arguments */
		duk_error(ctx, DUK_RET_TYPE_ERROR, dukky_error_fmt_argument, 4, dukky_argc);
	...
	if (dukky_argc > 0) {
		if (!duk_is_number(ctx, 0)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 0, "x0");
		}
	}
	if (dukky_argc > 1) {
		if (!duk_is_number(ctx, 1)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 1, "y0");
		}
	}
	if (dukky_argc > 2) {
		if (!duk_is_number(ctx, 2)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 2, "x1");
		}
	}
	if (dukky_argc > 3) {
		if (!duk_is_number(ctx, 3)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 3, "y1");
		}
	}
	/* Get private data for method */
	canvas_rendering_context2d_private_t *priv = NULL;
...
static duk_ret_t dukky_canvas_rendering_context2d_createRadialGradient(duk_context *ctx)
{
	...
	if (dukky_argc > 2) {
		if (!duk_is_number(ctx, 2)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 2, "r0");
		}
	}
	if (dukky_argc > 3) {
		if (!duk_is_number(ctx, 3)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 3, "x1");
		}
	}
	if (dukky_argc > 4) {
		if (!duk_is_number(ctx, 4)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 4, "y1");
		}
	}
	if (dukky_argc > 5) {
		if (!duk_is_number(ctx, 5)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 5, "r1");
		}
	}
	/* Get private data for method */
	canvas_rendering_context2d_private_t *priv = NULL;
...
static duk_ret_t dukky_canvas_rendering_context2d_clearRect(duk_context *ctx)
{
	...
	if (dukky_argc > 2) {
		if (!duk_is_number(ctx, 2)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 2, "w");
		}
	}
	if (dukky_argc > 3) {
		if (!duk_is_number(ctx, 3)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 3, "h");
		}
	}
	/* Get private data for method */
	canvas_rendering_context2d_private_t *priv = NULL;
...
static duk_ret_t dukky_canvas_rendering_context2d_fillText(duk_context *ctx)
{
	...
	if (dukky_argc > 1) {
		if (!duk_is_number(ctx, 1)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 1, "x");
		}
	}
	if (dukky_argc > 2) {
		if (!duk_is_number(ctx, 2)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 2, "y");
		}
	}
	if (dukky_argc > 3) {
		if (!duk_is_number(ctx, 3)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 3, "maxWidth");
		}
	}
	/* Get private data for method */
	canvas_rendering_context2d_private_t *priv = NULL;
...
static duk_ret_t dukky_canvas_rendering_context2d_drawImage(duk_context *ctx)
{
	...
	if (dukky_argc > 1) {
		if (!duk_is_number(ctx, 1)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 1, "dx");
		}
	}
	if (dukky_argc > 2) {
		if (!duk_is_number(ctx, 2)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 2, "dy");
		}
	}
	/* Get private data for method */
	canvas_rendering_context2d_private_t *priv = NULL;
...
static duk_ret_t dukky_canvas_rendering_context2d_createImageData(duk_context *ctx)
{
	...
	if (dukky_argc > 0) {
		if (!duk_is_number(ctx, 0)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 0, "sw");
		}
	}
	if (dukky_argc > 1) {
		if (!duk_is_number(ctx, 1)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 1, "sh");
		}
	}
	/* Get private data for method */
	canvas_rendering_context2d_private_t *priv = NULL;
...
static duk_ret_t dukky_canvas_rendering_context2d_getImageData(duk_context *ctx)
{
	...
	if (dukky_argc > 0) {
		if (!duk_is_number(ctx, 0)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 0, "sx");
		}
	}
	if (dukky_argc > 1) {
		if (!duk_is_number(ctx, 1)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 1, "sy");
		}
	}
	if (dukky_argc > 2) {
		if (!duk_is_number(ctx, 2)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 2, "sw");
		}
	}
	if (dukky_argc > 3) {
		if (!duk_is_number(ctx, 3)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 3, "sh");
		}
	}
	/* Get private data for method */
	canvas_rendering_context2d_private_t *priv = NULL;
...
static duk_ret_t dukky_canvas_rendering_context2d_quadraticCurveTo(duk_context *ctx)
{
	...
	if (dukky_argc > 0) {
		if (!duk_is_number(ctx, 0)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 0, "cpx");
		}
	}
	if (dukky_argc > 1) {
		if (!duk_is_number(ctx, 1)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 1, "cpy");
		}
	}
	if (dukky_argc > 2) {
		if (!duk_is_number(ctx, 2)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 2, "x");
		}
	}
	if (dukky_argc > 3) {
		if (!duk_is_number(ctx, 3)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 3, "y");
		}
	}
	/* Get private data for method */
	canvas_rendering_context2d_private_t *priv = NULL;
...
static duk_ret_t dukky_canvas_rendering_context2d_bezierCurveTo(duk_context *ctx)
{
	...
	if (dukky_argc > 0) {
		if (!duk_is_number(ctx, 0)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 0, "cp1x");
		}
	}
	if (dukky_argc > 1) {
		if (!duk_is_number(ctx, 1)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 1, "cp1y");
		}
	}
	if (dukky_argc > 2) {
		if (!duk_is_number(ctx, 2)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 2, "cp2x");
		}
	}
	if (dukky_argc > 3) {
		if (!duk_is_number(ctx, 3)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 3, "cp2y");
		}
	}
	if (dukky_argc > 4) {
		if (!duk_is_number(ctx, 4)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 4, "x");
		}
	}
	if (dukky_argc > 5) {
		if (!duk_is_number(ctx, 5)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 5, "y");
		}
	}
	/* Get private data for method */
	canvas_rendering_context2d_private_t *priv = NULL;
...
static duk_ret_t dukky_canvas_rendering_context2d_arcTo(duk_context *ctx)
{
	...
	if (dukky_argc < 5) {
		/* not enough arguments */
		duk_error(ctx, DUK_RET_TYPE_ERROR, dukky_error_fmt_argument, 5, dukky_argc);
	...
	if (dukky_argc > 0) {
		if (!duk_is_number(ctx, 0)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 0, "x1");
		}
	}
	if (dukky_argc > 1) {
		if (!duk_is_number(ctx, 1)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 1, "y1");
		}
	}
	if (dukky_argc > 2) {
		if (!duk_is_number(ctx, 2)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 2, "x2");
		}
	}
	if (dukky_argc > 3) {
		if (!duk_is_number(ctx, 3)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 3, "y2");
		}
	}
	if (dukky_argc > 4) {
		if (!duk_is_number(ctx, 4)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 4, "radius");
		}
	}
	/* Get private data for method */
	canvas_rendering_context2d_private_t *priv = NULL;
...
static duk_ret_t dukky_canvas_rendering_context2d_arc(duk_context *ctx)
{
	...
	if (dukky_argc > 2) {
		if (!duk_is_number(ctx, 2)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 2, "radius");
		}
	}
	if (dukky_argc > 3) {
		if (!duk_is_number(ctx, 3)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 3, "startAngle");
		}
	}
	if (dukky_argc > 4) {
		if (!duk_is_number(ctx, 4)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 4, "endAngle");
		}
	}
	if (dukky_argc > 5) {
		if (!duk_is_boolean(ctx, 5)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_bool_type, 5, "anticlockwise");
		}
	}
	/* Get private data for method */
	canvas_rendering_context2d_private_t *priv = NULL;
...
static duk_ret_t dukky_canvas_rendering_context2d_ellipse(duk_context *ctx)
{
	...
	if (dukky_argc < 7) {
		/* not enough arguments */
		duk_error(ctx, DUK_RET_TYPE_ERROR, dukky_error_fmt_argument, 7, dukky_argc);
	...
	if (dukky_argc > 2) {
		if (!duk_is_number(ctx, 2)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 2, "radiusX");
		}
	}
	if (dukky_argc > 3) {
		if (!duk_is_number(ctx, 3)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 3, "radiusY");
		}
	}
	if (dukky_argc > 4) {
		if (!duk_is_number(ctx, 4)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 4, "rotation");
		}
	}
	if (dukky_argc > 5) {
		if (!duk_is_number(ctx, 5)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 5, "startAngle");
		}
	}
	if (dukky_argc > 6) {
		if (!duk_is_number(ctx, 6)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 6, "endAngle");
		}
	}
	if (dukky_argc > 7) {
		if (!duk_is_boolean(ctx, 7)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_bool_type, 7, "anticlockwise");
		}
	}
	/* Get private data for method */
	canvas_rendering_context2d_private_t *priv = NULL;
