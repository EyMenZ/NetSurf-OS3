...
static duk_ret_t dukky_path2d_addText(duk_context *ctx)
{
	...
	if (dukky_argc > 3) {
		if (!duk_is_number(ctx, 3)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 3, "x");
		}
	}
	if (dukky_argc > 4) {
		if (!duk_is_number(ctx, 4)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 4, "y");
		}
	}
	if (dukky_argc > 5) {
		if (!duk_is_number(ctx, 5)) {
			duk_error(ctx, DUK_ERR_ERROR, dukky_error_fmt_number_type, 5, "maxWidth");
		}
	}
	/* Get private data for method */
	path2d_private_t *priv = NULL;
